...
def extract_wallet_ids(leaderboard_url):
    ...
    if response.status_code != 200:
        return []  # Return an empty list if the page could not be loaded
    ...
    for a_tag in a_tags:
        ...
        if href.startswith('/profile/'):
            # Extract wallet ID from the href
            wallet_id = href.split('/')[-1]
            wallet_ids.append(wallet_id)

    return wallet_ids  # Return the list of extracted wallet IDs
...
def get_active_positions(all_positions: pd.DataFrame) -> pd.DataFrame:
    try: all_positions['endDate'] = pd.to_datetime(all_positions['endDate'])
    except: return None
...
def position_over_value(user_address: str, asset: str) -> None:
    ...
    try: return specific_open_position['size'].iloc[0]
    except: return 0
...
def send_to_tail_trades(tail_trade: bool, trade_data: dict, file_path: str = 'tail_trades.json'):
    if tail_trade:
        ...
        # Write the updated list back to the file, remeber there are multiple processes
        with open(file_path, 'w') as file:
            with portalocker.Lock(file, mode='w'):
